...
description: "Find how to easily test against Spark+Delta catalogs with this new Docker container"
date: 2025-10-23
categories: [spark, delta, docker]
image: docker_whale.webp
---

> **Pre-requisite:** if you’re not familiar with Docker, I recommend you follow the [official introductory guide](https://docs.docker.com/get-started/introduction/).
...
Unfortunately, the traditional method of installing the OpenJDK, JVM, Spark drivers, Hive plugins, Delta jars is anything but easy, let alone reproducible *(As a matter of fact, on the eve of publishing this article, [the Live Notebooks on PySpark’s official quickstart page](https://spark.apache.org/docs/latest/api/python/getting_started/index.html) failed to launch due to a bug downloading OpenJDK… 😅)*.

Faced with this problem, I decided to take matters into my own hands — and published [a Docker Hub repository for running Spark Connect servers](https://hub.docker.com/repository/docker/franciscoabsampaio/spark-connect-server)!
...
We use the “delta” tag if we wish to spin the server with a [Delta catalog](https://delta.io/), and the “iceberg” tag for the [Iceberg catalog](https://iceberg.apache.org/).
...
The yield statement locks the fixture execution until the test finishes — at which point the cleanup code is executed.
...
You can find the container on Docker Hub: 👉 [franciscoabsampaio/spark-connect-server](https://hub.docker.com/repository/docker/franciscoabsampaio/spark-connect-server)
